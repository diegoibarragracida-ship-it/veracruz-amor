...
const PrestadoresAdmin = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [regRes, solRes, prestRes] = await Promise.all([
  ...
  const handleAprobarRegistro = async (registroId) => {
    try {
  ...
  const handleRechazarRegistro = async (registroId) => {
    try {
  ...
  const pendingCount = registros.length + solicitudes.length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        <Button
          ...
          className={tab === "registros" ? "bg-[#1B5E20]" : ""}
        ...
        >
          Solicitudes Encargados ({solicitudes.length})
        </Button>
        <Button
        ...
        <div className="bg-green-50 border border-green-200 rounded-xl p-4" data-testid="credentials-banner">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-green-600 mt-1">(Envío de email simulado - en producción se enviarían por correo)</p>
            </div>
            <button onClick={() => setApprovedCredentials(null)} className="text-green-400 hover:text-green-600">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
      ...
      {/* Tab: Registros (self-registrations) */}
      {tab === "registros" && (
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          {registros.length === 0 ? (
            <div className="p-12 text-center text-gray-500">
              No hay registros de prestadores pendientes
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {registros.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell className="font-medium">{r.nombre_negocio}</TableCell>
                    <TableCell>
                      <Badge className="bg-blue-100 text-blue-800">{r.tipo}</Badge>
                    </TableCell>
                    <TableCell>
                      <div className="text-sm">
                        <p>{r.nombre_contacto}</p>
                        <p className="text-gray-500">{r.email_contacto}</p>
                      </div>
                    </TableCell>
                    <TableCell>{r.municipio_nombre}</TableCell>
                    <TableCell>
                      <Badge className="bg-gray-100 text-gray-800">{r.documentos?.length || 0}</Badge>
                    </TableCell>
                    <TableCell>{new Date(r.fecha_registro).toLocaleDateString()}</TableCell>
                    <TableCell>
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => setSelectedRegistro(r)} data-testid={`view-registro-${r.id}`}>
                          <Eye className="w-4 h-4" />
                        </Button>
                        <Button size="sm" className="bg-green-600 hover:bg-green-700" onClick={() => handleAprobarRegistro(r.id)} data-testid={`approve-registro-${r.id}`}>
                          <Check className="w-4 h-4" />
                        </Button>
                        <Button size="sm" variant="outline" className="text-red-600" onClick={() => handleRechazarRegistro(r.id)}>
      ...
      {/* Tab: Solicitudes (from encargados) */}
      {tab === "solicitudes" && (
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          {solicitudes.length === 0 ? (
            <div className="p-12 text-center text-gray-500">
              No hay solicitudes pendientes de encargados
            </div>
          ) : (
            <Table>
              <TableHeader>
      ...
      {/* Detail Dialog for registro */}
      <Dialog open={!!selectedRegistro} onOpenChange={() => setSelectedRegistro(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Detalle de solicitud: {selectedRegistro?.nombre_negocio}</DialogTitle>
          </DialogHeader>
          {selectedRegistro && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedRegistro.tipo}{selectedRegistro.subtipo ? ` - ${selectedRegistro.subtipo}` : ""}</p>
                </div>
                <div>
                  <p className="text-gray-500">Municipio</p>
                  <p className="font-medium">{selectedRegistro.municipio_nombre}</p>
                </div>
                <div>
                  <p className="text-gray-500">Responsable</p>
                  <p className="font-medium">{selectedRegistro.nombre_contacto}</p>
                </div>
                <div>
                  <p className="text-gray-500">Email</p>
                  <p className="font-medium">{selectedRegistro.email_contacto}</p>
                </div>
                <div>
                  <p className="text-gray-500">Teléfono</p>
                  <p className="font-medium">{selectedRegistro.telefono || "N/A"}</p>
                </div>
                <div>
                  <p className="text-gray-500">WhatsApp</p>
                  <p className="font-medium">{selectedRegistro.whatsapp || "N/A"}</p>
                </div>
              </div>
              ...
              {selectedRegistro.descripcion && (
                <div>
                  ...
                  <p className="text-sm">{selectedRegistro.descripcion}</p>
                </div>
              )}
              ...
              {selectedRegistro.documentos?.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {selectedRegistro.documentos.map((docPath, i) => (
                      <a
                        key={i}
                        href={`${process.env.REACT_APP_BACKEND_URL}/api/files/${docPath}`}
                        target="_blank"
          ...
          <DialogFooter>
            ...
            <Button variant="outline" className="text-red-600 border-red-300 hover:bg-red-50" onClick={() => handleRechazarRegistro(selectedRegistro?.id)}>
              <XCircle className="w-4 h-4 mr-1" /> Rechazar
            </Button>
            <Button className="bg-green-600 hover:bg-green-700" onClick={() => handleAprobarRegistro(selectedRegistro?.id)}>
