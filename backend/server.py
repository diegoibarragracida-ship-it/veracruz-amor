...
@api_router.put("/reservas/{reserva_id}/checkin")
async def checkin_reserva(reserva_id: str, current_user: dict = Depends(get_current_user)):
    await db.reservas.update_one(
        {"id": reserva_id},
        {"$set": {"checkin_at": datetime.now(timezone.utc).isoformat(), "estado": "en_curso"}}
    )
    return {"ok": True}

@api_router.put("/reservas/{reserva_id}/checkout")
async def checkout_reserva(reserva_id: str, current_user: dict = Depends(get_current_user)):
    await db.reservas.update_one(
        {"id": reserva_id},
        {"$set": {"checkout_at": datetime.now(timezone.utc).isoformat(), "estado": "completada"}}
    )
    return {"ok": True}

# ============== TARIFAS POR TEMPORADA ==============

@api_router.get("/prestadores/{prestador_id}/tarifas")
async def get_tarifas(prestador_id: str):
    tarifas = await db.tarifas.find({"prestador_id": prestador_id}, {"_id": 0}).to_list(100)
    return {"tarifas": tarifas}

@api_router.post("/prestadores/{prestador_id}/tarifas")
async def create_tarifa(prestador_id: str, request: Request, current_user: dict = Depends(get_current_user)):
    body = await request.json()
    tarifa = {
        "id": str(uuid.uuid4()),
        "prestador_id": prestador_id,
        **body,
        "multiplicador": float(body.get("multiplicador", 1.5)),
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
    await db.tarifas.insert_one(tarifa)
    tarifa.pop("_id", None)
    return tarifa

@api_router.delete("/prestadores/{prestador_id}/tarifas/{tarifa_id}")
async def delete_tarifa(prestador_id: str, tarifa_id: str, current_user: dict = Depends(get_current_user)):
    await db.tarifas.delete_one({"id": tarifa_id, "prestador_id": prestador_id})
    return {"ok": True}

# ============== CUPONES ==============

@api_router.get("/prestadores/{prestador_id}/cupones")
async def get_cupones(prestador_id: str):
    cupones = await db.cupones.find({"prestador_id": prestador_id}, {"_id": 0}).to_list(100)
    return {"cupones": cupones}

@api_router.post("/prestadores/{prestador_id}/cupones")
async def create_cupon(prestador_id: str, request: Request, current_user: dict = Depends(get_current_user)):
    body = await request.json()
    # Verificar código único
    existing = await db.cupones.find_one({"prestador_id": prestador_id, "codigo": body.get("codigo","").upper()})
    if existing:
    ...
    await db.cupones.insert_one(cupon)
    cupon.pop("_id", None)
    return cupon

@api_router.delete("/prestadores/{prestador_id}/cupones/{cupon_id}")
async def delete_cupon(prestador_id: str, cupon_id: str, current_user: dict = Depends(get_current_user)):
    await db.cupones.delete_one({"id": cupon_id, "prestador_id": prestador_id})
    return {"ok": True}

@api_router.post("/cupones/validar")
async def validar_cupon(request: Request):
    body = await request.json()
    cupon = await db.cupones.find_one({
    ...
    if cupon.get("fecha_fin") and cupon["fecha_fin"] < datetime.now(timezone.utc).date().isoformat():
        raise HTTPException(status_code=400, detail="Cupón expirado")
    if cupon.get("usos_actuales", 0) >= cupon.get("max_usos", 999):
        raise HTTPException(status_code=400, detail="Cupón agotado")
    return {"valido": True, "descuento_pct": cupon["descuento_pct"], "descripcion": cupon.get("descripcion","")}

# ============== ESTADO LIMPIEZA HABITACIÓN ==============
...
async def startup_event():
    ...
    await db.tarifas.create_index("prestador_id")
    await db.cupones.create_index("prestador_id")
    await db.cupones.create_index("codigo")
